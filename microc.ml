...
let print_helper x y = print_endline(x ^ " " ^ y ^ ",")
...
and
 eval mymap funcMap (expr: Ast.expr) = match expr with
  ...
  | Binop(e1, op, e2) ->
      ...
      match op with
    	Add -> (string_of_int (int_of_string v1 + int_of_string v2), mymap2, funcMap2)
      | Sub -> (string_of_int (int_of_string v1 - int_of_string v2), mymap2, funcMap2)
      | Mult -> (string_of_int (int_of_string v1 * int_of_string v2), mymap2, funcMap2)
      | Div -> (string_of_int (int_of_string v1 / int_of_string v2), mymap2, funcMap2)
...
let () =
  ...
  let hd::tl = List.rev funcs in
  let (a,_,_) = (exec_stmt_list StringMap.empty funcMap hd.body) in
  print_endline(a)
